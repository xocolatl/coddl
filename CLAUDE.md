...
- **`Tuple {}` is the unit type; `{}` is its one value.** An `oper` without an explicit return clause implicitly returns `Tuple {}`. The literal `{}` in expression position evaluates to the unit value. Same role as `()` in Rust, `Void` in Swift. Don't propose a separate `Unit` or `Void` keyword. See ARCHITECTURE.md §7.
