...
| Type           | Fields                                                  |
|----------------|---------------------------------------------------------|
| `Module`       | `program_name: String`, `functions: Vec<Function>`, `headings: Vec<Heading>`, `public_relvars: Vec<PublicRelvarBinding>`, `db_path_default: Option<String>`, `db_name: Option<String>` |
...
| `Expr::NameRef` (public relvar) | When the plan supplies a public relvar whose surface name matches the NameRef, the lowerer emits `Inst::RelvarRead { dst, name, heading_id }` (a slot load + retain) and records `dst`'s `ProcType::Relation(heading_id)`. The typechecker has already enforced this only happens inside a `transaction [...]` (T0025). Consumers (`where` / `extract` / `write_relation`) release the temporary via the existing fresh-source detection. |
