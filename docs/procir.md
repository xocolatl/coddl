...
| `RegisterPlan`       | `plan_id: u32`                                                             | —                     | Register one baked plan (looked up by id in `Module::plans`). Backend calls `coddl_register_plan(plan_id, db_name, sql, param_count, rel_arities, n_rels, &@.heading.<result>)` — `rel_arities` is a static `u32` array (one arity per relation-valued parameter slot; null when the plan ships no relation). Emitted once per `Module::plans` entry in `main`'s prologue, after `RegisterDatabase`. |
