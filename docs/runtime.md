...
- The program prologue registers each logical database once, then each static plan:
  ...
  - `coddl_register_plan(plan_id, db_name, sql, param_count, rel_arities, n_rels, result_desc)` — the plan references its database by name. `rel_arities` names the arity of each relation-valued parameter slot (null/0 for an ordinary plan). No separate parameter-type table: scalar bind parameters self-describe via `CoddlParam.kind` at the call site, relation parameters via their heading descriptor.
- At the force point, codegen calls `coddl_query(plan_id, params, n, rels, n_rels) -> *Relation`: it resolves the plan's database, substitutes each relation-valued parameter's `__CODDL_REL_<slot>__` marker with a table primary built from the bound relation's rows (`(VALUES …)` of numbered groups whose cells bind after the scalar params, or a typed zero-row SELECT for an empty relation — never a `NULL` token), fires the prepared statement (cached by expanded SQL text per connection) on a pool connection — so the audit `trace` hook captures it — marshals the rows into an RC relation (no seal: the query's `DISTINCT`/key already makes the rows a set), and returns the pointer (the same shape `coddl_relation_where` returns; consumed by `coddl_extract_check_cardinality`). It aborts on any hard error (unknown plan/database, parameter count/arity mismatch, over the bind ceiling — a read never batch-splits — prepare/step failure, NULL cell); the `*Relation` return has no status channel. (`TempRelRef` temp-table shipping remains the designed escalation for relations too large for a bound `VALUES` list.)
