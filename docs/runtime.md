...
- The program prologue registers each logical database once, then each static plan:
  ...
  - `coddl_register_plan(plan_id, db_name, sql, param_count, rel_specs, n_rels, result_desc, card1_alt, dispatch_slot, gate_params, n_gates)` — the plan references its database by name. `rel_specs` is one interleaved `[arity, flags]` pair per relation-valued parameter slot (flags bit 0 = absorbs-empty; null/0 for an ordinary plan); `card1_alt` (−1 = none) links the plan's cardinality-1 sibling and `dispatch_slot` names the slot whose runtime count selects it; `gate_params` is the array of 0-based scalar-bind indices of the `when`-gate conjuncts on the absorbing spine (null/0 for a plan with no absorbing gate; each index validated `< param_count` at registration). No separate parameter-type table: scalar bind parameters self-describe via `CoddlParam.kind` at the call site, relation parameters via their heading descriptor.
- At the force point, codegen calls `coddl_query(plan_id, params, n, rels, n_rels) -> *Relation`. Holding every shipped relation *and* every scalar bind, it **dispatches on the knowledge it already has**: an empty relation at an *absorbing* slot returns a fresh empty relation on the plan's result descriptor **without firing a statement** (no prepare, no connection touch — observationally identical, RM Pre 8); a registered gate bind of 0 does the same (the **false-gate statement skip**, the Boolean sibling of the empty-slot check — it runs before the cardinality dispatch, so a false gate never reaches the sibling); exactly one row at the dispatch slot fires the registered cardinality-1 sibling instead, binding the plan's scalar args (`?1..?k`) then the row's cells (`?k+1..?k+m`, canonical order); otherwise it resolves the plan's database, substitutes each relation-valued parameter's `__CODDL_REL_<slot>__` marker with a table primary built from the bound relation's rows (`(VALUES …)` of numbered groups whose cells bind after the scalar params, or a typed zero-row SELECT for an empty non-absorbing relation — never a `NULL` token), fires the prepared statement (cached by expanded SQL text per connection) on a pool connection — so the audit `trace` hook captures it — marshals the rows into an RC relation (no seal: the query's `DISTINCT`/key already makes the rows a set), and returns the pointer (the same shape `coddl_relation_where` returns; consumed by `coddl_extract_check_cardinality`). Past the read bind ceiling the general form **escalates** instead of expanding (a read never batch-splits): every slot's marker substitutes to a stable per-(plan, slot) session temp table (`coddl_rp_<plan>_<slot>`, bare `column1…N` columns — `VALUES`-identical affinity), each populated (clear + insert-budget-batched `INSERT`) and cleared after the read under one pool-guard span, and the statement fires with scalar binds only — the escalated text is stable, so one cached prepared statement serves every escalated fire (`fire_escalated`; see [sqlemit.md](sqlemit.md) "Sending in-memory relations back into SQL"). It aborts on any hard error (unknown plan/database, parameter count/arity mismatch, scalar binds alone over the ceiling — nothing to escalate — prepare/step failure, NULL cell); the `*Relation` return has no status channel.
...
| `coddl_relation_when`      | `(rel, cond: i8, desc) -> ptr`                      | The `when` gate (`R times ⟨c⟩`). `cond ≠ 0`: retain `rel` and return it — the result *is* the operand, no copy. `cond = 0`: a fresh empty relation with the operand's heading (`coddl_rc_alloc(0, 0, Relation, desc)`). O(1) either way; no re-seal (the result is the sealed operand or empty). Borrows the operand, returns an owned (+1) payload like every relation operator. |
| `coddl_relation_otherwise` | `(primary, fallback) -> ptr`                        | Relational COALESCE (`otherwise`): header-length check on `primary`; retain and return the winner (`primary` if nonempty, else `fallback`). O(1), no descriptor needed, no re-seal — the result *is* one of the operands, already a sealed set. Borrows both operands, returns an owned (+1) payload. |
