...
| The `when` gate (`Predicate::Gate`) renders as the tuple-independent conjunct **`?N = 1`** (`$N = 1` on Postgres) in the coalesced `WHERE`, its Boolean bound as integer 0/1 like any scalar param — never a bare truthy placeholder (`WHERE ?N` is valid SQLite but a type error as `WHERE $N` on Postgres), and never inlined text. The condition value is a `ParamSource::Param` — a bare local under its own name, a computed condition under the build's `__when_<k>` binding (see [relir.md](relir.md)). v1 runs the statement even when the gate binds false (SQLite evaluates the conjunct per row and returns nothing); the **force-time statement skip** — check the gate binds before preparing, return the empty result with *no statement*, the Boolean sibling of the empty-slot short-circuit — is the tracked follow-up (`.local/tracking/optimizations.md`). | Performance (docs/principles.md §1): the gate ships as a bind, never a pull of the relvar to gate in-process; dialect portability. |
