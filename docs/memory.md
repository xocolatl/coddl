...
**`extract` (and boxed-tuple unbox) defer the source.** `extract` copies a record's cells into a tuple as borrowed values, so the source relation's release is deferred (after every use of the borrowed fields) rather than freed immediately — the drop walker would otherwise dangle the fields. A small-tuple call result is `TupleUnbox`ed the same way: the boxed record is deferred-released after the borrowed flattened fields are consumed. **A deferred release must stay dominated by its producer:** an entry created inside an `if`-arm releases at *that arm's exit* (which the arm dominates), never at the `if`-merge / function epilogue (which it doesn't) — so a bound-and-field-read call result inside a branch (`let r = mk{}; … r.field …`, the wiki `/wiki/{slug}` route) lowers, and a *sibling* arm's early `return` never drains it. The one case that stays deferred to the epilogue: an unbox'd result that is the arm's **escaping tail** (`[ let r = mk{}; r ]` — the raw box flows through the merge), which still needs merge-carried box ownership and is not yet lowered.
