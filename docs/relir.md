...
- The per-node **FD set** and **constraint set** (only heading, origin, and leaf keys exist today). **Key inference through the binary nodes** is the near-term piece: `surviving_keys()` returns empty for `And`/`Or`/`Minus`, so every `join`/`compose`/`intersect`/`union` emits a redundant `DISTINCT`. Closing this is required by the "our optimizer does all the work" rule ([principles.md](principles.md) §1) — we elide `DISTINCT` ourselves, never leaving it to the backend planner. The soundness rules and running catalog live in [sqlemit.md](sqlemit.md) ("`DISTINCT` elision"); `Semijoin` already propagates `lhs`'s keys (a filter can't duplicate), and the `And` rule (a key survives a natural join when the join attributes cover a key of the other operand) is the next to land.
